...
static inline void start_comm_led_drivers() {
    HAL_GPIO_WritePin(GPIO_LED_DRIVERS, GPIO_PIN_OE_OPTO, GPIO_PIN_SET);
...
static inline void start_comm_OPTO() {
    HAL_GPIO_WritePin(GPIO_LED_DRIVERS, GPIO_PIN_OE_OPTO, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIO_LED_DRIVERS, GPIO_PIN_LE_OPTO, GPIO_PIN_SET);
}

static inline void start_comm_IND() {
    HAL_GPIO_WritePin(GPIO_LED_DRIVERS, GPIO_PIN_OE_IND, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIO_LED_DRIVERS, GPIO_PIN_LE_IND, GPIO_PIN_SET);
}

static inline void start_comm_ADC(uint8_t idx) {
...
static inline void write_OPTO(uint8_t optoIdx) {
    HAL_SPI_Transmit(SPI_INSTANCE, (uint8_t*)OPTO_BUFFERS[optoIdx], NUM_OPTOS / 8, 2);  // selects every 8th optical sensor
}

static inline void write_IND(const uint8_t *leds) {
    uint8_t ind_buffer[NUM_OPTOS / 8 * 2];
    ...
    memset(ind_buffer, 0, NUM_OPTOS / 8);
    memcpy(&ind_buffer[NUM_OPTOS / 8], leds, NUM_OPTOS / 8);

    HAL_SPI_Transmit(SPI_INSTANCE, ind_buffer, NUM_OPTOS / 8 * 2, 2);
}
...
static inline void end_comm_led_drivers() {
    HAL_GPIO_WritePin(GPIO_LED_DRIVERS, GPIO_PIN_LE_OPTO, GPIO_PIN_RESET);
