...
static inline int8_t opto_idx_to_pos_mm(float optoIdx) {
    return (int8_t)round((optoIdx - MID_OPTO_POS) * DIST_OPTO_MM);
}

static inline uint8_t pos_mm_to_opto_idx(float pos_mm) {
    return (uint8_t)clamp(round(MID_OPTO_POS + pos_mm / DIST_OPTO_MM), 0, NUM_OPTOS - 1);
}
...
void linepos_calc2(LinePosCalc *data, const uint8_t *measurements) {
    ...
    static const uint8_t black_delta = 50;
    ...
    for (uint8_t i = 0; i < NUM_OPTOS + 1; ++i) {
        const uint8_t meas = i < NUM_OPTOS ? measurements[i] : 0;
        const Color color = meas >= avg_ + black_delta ? Color_BLACK : meas >= avg_ ? Color_GREY : Color_WHITE;
...
//    str[strIdx++] = '\n';
    ...
    if (lines->numLines) {
        ...
        pos = strlen(str);
        for (uint8_t i = 0; i < lines->numLines; ++i) {
            itoa(lines->positions_mm[i], &str[pos], 10);
            pos = strlen(str);
